...
public:
	...
	~EM();
	int pagecap;	
	void EMCluster(char *pinfile, char *poutfile, char *porderfile, double pepsilon, double pdistepsilon, double pmindensity);
private:
	...
	double *resultmem;
	double *N;
	double Det(double *matrix, int pdim);
	double setcoredistance(double pN, double pmindensity, int pneighcount, double *pneighdif, int *pneighindex);
	void popseedlist(int *pseedlist, double *pseeddif, int *pcluster2seed, int seedcount);
	void expandclusterorder(int pclusternum, double pdisteps, double pmindenstiy, int *pprocessed, FILE *pofp);
	void orderseedlist(int pseedcount, int *pseedlist, double *pseeddif, int *pcluster2seed);
	int updatedif(int *pseedflag, int *pcluster2seed, int *pseedlist, double *pseeddif, int *pneighindex, double *pneighdif, int pseedcount, int pneighcount);
	int neighbour(int clusternum, double pdistepsi, int *pneighindex, double *pneighdif, int *pprocessed);
	void optics(char *poutfile, double pdistepsi, double pmindensity);
	double probdifference(int numcluster1, int numcluster2);
	void filewrite(char *poutfile, double *pu, double *psigma, int pdim, int pnumcluster);
    int dim;
	int numcluster;
	double *RS; 
	char *infile, *outfile;
	...
	void eigenvalue(FILE *pofp, double *matrix, int pdim);
    void InverseMatrix(double *pmatrix,int pmatrixdim, int pnumcluster);
};
...
void main(int argc, char **argv) {
	...
	int mnumcluster;
	int mpagecap;
	double mepsilon, mdistepsilon, mmindensity;
	Timer excutetime;
...
/*    minfile="wdbcn5.txt";
	...
	minfile="data_norm.txt";   // Data file
	mnumcluster=200;
	mdimension=36;     // Number of columns
	mpagecap=67336;     // Number of rows
	mepsilon=0;
	mmindensity = 20;
	mdistepsilon = 0;
	meanfile="data_seed.txt";
	moutfile="data_out.txt";
	orderfile="data_ord.txt";
...
EM::EM(char *ufile, int pnumcluster, int pdimension, int ppagecap){
	...
	RS = (double *) malloc (sizeof (double) * dim);
	u = (double * ) malloc ( sizeof (double) * pnumcluster * dim);
	sigma = (double * ) malloc (sizeof (double) * pnumcluster * sigmasize);
	determinant = (double * ) malloc (sizeof (double) * pnumcluster);
	N = (double *) malloc (sizeof(double) * pnumcluster);
	tempmem = (double *) malloc (sizeof(double) * dim * dim);
	resultmem = (double *) malloc (sizeof(double) * dim * dim);
	eigmem = (double *) malloc (sizeof(double) * dim * (dim+4));
...
void EM::InverseMatrix(double *pmatrix, int pmatrixdim, int pnumcluster){
	double *tempmatrix;
	int i,j,k,m;
	int count;
	double temp;
...
//	tempmatrix = (double *) malloc (sizeof(double) * pmatrixdim * pmatrixdim);
//	resultmatrix = (double *) malloc (sizeof(double) * pmatrixdim * pmatrixdim);
...
void EM::eigenvalue(FILE *pofp, double *matrix, int pdim){
	int i, j, k;
	double maxelement;
	int p, q;
	double *S;
	...
	double m,n;
	double *temprowp, *temprowq, *tempcolp, *tempcolq;
	...
	while (fabs(maxelement)>eigeneps){
		...
		for (i=0; i<pdim; i++){
			...
			if (i==p){
				...
				temprowp[i]=double(matrix[matrix_index1]*pow(cos(thita),2)
					+matrix[matrix_index2]*pow(sin(thita),2)+matrix[matrix_index]*sin(2*thita));
				tempcolp[i]=temprowp[i];
				temprowq[i]=double((matrix[matrix_index2]-matrix[matrix_index1])*sin(2*thita)/2+matrix[matrix_index]*cos(2*thita));
			...
			//bqq,bpq,bqp
				...
				if (p < q)
					...
					matrix_index = (p + 1) * p / 2+ q;
				temprowq[i]=double(matrix[matrix_index1]*pow(sin(thita),2)
					+matrix[matrix_index2]*pow(cos(thita),2)-matrix[matrix_index]*sin(2*thita));
				tempcolq[i]=temprowq[i];
				temprowp[i]=double((matrix[matrix_index2]-matrix[matrix_index1])*sin(2*thita)/2+matrix[matrix_index]*cos(2*thita));
				tempcolp[i]=temprowp[i];					
			}
				...
				temprowp[i]=double(matrix[matrix_index1]*cos(thita)+matrix[matrix_index2]*sin(thita));
				temprowq[i]=double(-matrix[matrix_index1]*sin(thita)+matrix[matrix_index2]*cos(thita));
				...
				tempcolp[i]=double(matrix[matrix_index1]*cos(thita)+matrix[matrix_index2]*sin(thita));
				tempcolq[i]=double(-matrix[matrix_index1]*sin(thita)+matrix[matrix_index2]*cos(thita));
			}
		}//for i<pdim
		...
		for (i=0; i<pdim; i++){
			tempcolp[i]=double(S[i*pdim+p]*cos(thita)+S[i*pdim+q]*sin(thita));
			tempcolq[i]=double(S[i*pdim+p]*(-sin(thita))+S[i*pdim+q]*cos(thita));
		}
    ...
    */
	...
	for (i=0; i<pdim; i++)
		for (j=0; j<pdim; j++)
			fprintf(pofp, "%.18lf ",S[j*pdim+i] * 255);
	fprintf(pofp, "\n");	
}
...
/*******************************EM Clustering**************************************/
void EM::EMCluster(char *pinfile, char *poutfile, char *porderfile, double pepsilon, double pdistepsilon, double pmindensity){
	...
	tempv = (double *) malloc (sizeof (double) * dim);
	PRxl = (double *) malloc (sizeof (double) * numcluster);
	PRlx = (double *) malloc (sizeof (double) * samplesize * pagecap);
	uXXT = (double *) malloc (sizeof (double) * numcluster * dim * (dim+1)/2);
	uX = (double *) malloc (sizeof (double) * numcluster * dim);
	dist = (double *) malloc (sizeof (double) * numcluster * (numcluster-1)/2);
	tempPRlx = (double *) malloc (sizeof (double) * numcluster);
...
//	emtime.start();
	...
	double g_Factor = double(pow(2 * 3.14, dim));
	if (g_Factor > 1000000)
		g_Factor = 10000000;
...
//				printf("the determinant of cluster %d is %f;\n", j, determinant[j]);
			if (iterationnum == 1)
				determinant[j] = double(sqrt(determinant[j]));
			else
				determinant[j] = double(sqrt(g_Factor * fabs(determinant[j])));
//			if (iterationnum != 1)
//				printf("the determinant of cluster %d after modify is %f;\n", j, determinant[j]);
        ...
        /***********************E-step for Singleton Records***************************/
				...
				for (j = 0; j < numcluster; j++){
					if (PRxl[j] < 600)
						PRxl[j] = double(exp(- PRxl[j] / 2)/ determinant[j]);
					else
						PRxl[j] = 0;
					}
				for (j = 0; j < numcluster; j++){
				...
				/* update newE */
				if (PRx > 0) newE = newE + double(log(PRx));
...
//					printf(" tmp_ItemList[%d].ItemList[%d] = %d\n", count, j,  tmp_ItemList[count].ItemList[j]);
        ...
        /***********************E-step for Singleton Records***************************/
				...
				for (j = 0; j < samplesize; j++){
					...
					PRxl[j] = double(fabs(PRxl[j]));  
					if (PRxl[j] < 600)
						PRxl[j] = double(exp(- PRxl[j] / 2) / determinant[tmp_cluster]);
					else
						PRxl[j] = 0; 
				}			
				...
				/* update newE */
				if (PRx > 0) newE = newE + double(log(PRx)/100);
    ...
    if ((fabs(E - newE) > epsilon) && (iterationnum < iterationtime)) {
  		for (j = 0; j < numcluster; j++){
  			if (N[j] + 1 == 1)
  				...
  				for (k = 0; k < dim; k++){
  					...
  					for (m = 0; m < dim; m++){
  						...
  						if (fabs(sigma[sigma_index]) < 0.001)
  							if (k == m) sigma[sigma_index] = double(0.001);						
...
/* Output data */
filewrite("classify.txt", u, sigma, dim, numcluster);
	...
	FILE *tt=fopen("membership.txt", "w");
	ifp = fopen(infile, "r");

	if (!tt || !ifp) {
		printf("Infile/membership file not opened!");		
		exit(0);
	}
	...
	for (i = 0; i < pagecap; i++)
	{
		...
		for (j = 0; j < dim; ++j) { fscanf(ifp, "%lf ", &RS[j]); }
  	fscanf(ifp, "\n");

    // Use Euclidean distance for unclassified points
    if (member == -1) {
      double mindist = 0, tmindist;
      double v;
...
//	emtime.end();
	...
	for (i = 0; i < pagecap; i++){
		for (j = 0; j < dim; j++){
			fscanf(ifp,"%lf ", &tempfval);
			fprintf(ofp, "%lf ", tempfval);
		}
		fscanf(ifp, "\n");
		fprintf(ofp, "\n");
	}
	...
	for (i = 0; i < numcluster; i++){
		...
		for (j = 0; j < dim; j++)
			fprintf(ofp, "%.18lf ", u[index * dim +j]);
		for (j = 0; j < dim; j++){
			fscanf(ifp, "%lf ", &reach);
			fprintf(ofp, "%.18lf ", reach);
		}
...
void EM::filewrite(char *poutfile, double *pu, double *psigma, int pdim, int pnumcluster)
{
	FILE *ofp;
	int i;
	...
	for (i = 0; i < pnumcluster; i++){
		fwrite(&pu[i * pdim], sizeof(double), pdim, ofp);
		fwrite(&psigma[i * sigmasize], sizeof(double), sigmasize, ofp);
	}

	fclose(ofp);
}

double EM::probdifference(int pnumcluster1, int pnumcluster2)
{
	double probdif;
	int i;
	...
	return double(probdif);
}

/* a subroutine called by optic */
/* pclusternum: current microcluster */
/* pprocessed: indicator array */
void EM::expandclusterorder(int pclusternum, double pdisteps, double pmindensity, int *pprocessed, FILE *pofp)
{
	int *neighindex;
	double *neighdif;
	int j;
...
//	printf("%d clusters processed\n", processedcluster);
	...
	fprintf(pofp, "%.18lf %d ", -1.000, pclusternum);
//	printf("cluster %d:\n", pclusternum);
...
//				PRlx[j * numcluster + pclusternum] = PRlx[j * numcluster +neighcluster];
		...
		fprintf(pofp, "%.18lf %d ", neighdif[0], neighcluster);
//		printf("cluster %d:\n", neighcluster);
...
void EM::optics(char *poutfile, double pdistepsi, double pmindensity)
{
...
int EM::neighbour(int clusternum, double pdistepsi, int *pneighindex, double *pneighdif, int *pprocessed)
{
	int i,j;
	double truedif, tempdif;
	int neighcount;
	int tempindex;
...
int EM::updatedif(int *pseedflag, int *pcluster2seed, int *pseedlist, double *pseeddif, int *pneighindex, double *pneighdif, int pseedcount, int pneighcount)
{
	int i;
	int seedindex;
	double newreach;
...
void EM::orderseedlist(int pseedcount, int *pseedlist, double *pseeddif, int *pcluster2seed)
{
	int i, j;
...
void EM::popseedlist(int *pseedlist, double *pseeddif, int *pcluster2seed, int seedcount)
{
	int k;
...
double EM::setcoredistance(double pN, double pmindensity, int pneighcount, double *pneighdif, int *pneighindex)
{
	...
	if (density < pmindensity)
		...
		distorder = (double *) malloc (sizeof(double) * pneighcount);
...
double EM::Det(double *matrix, int pdim){
	double determinant;
	double *tempmatrix;
	int flag;
	int i, j, k;
